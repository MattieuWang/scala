...
object WhatsWrong2 {
  ...
  def getCEOAndEnterprise(ceo_id: Option[String]): Future[(Option[CEO], Option[Enterprise])] = {
    ...
    val enterprise_promise = Promise[Option[Enterprise]]()
    val ceo_promise = Promise[Option[CEO]]()

    /*
    the key word Promise allows us to execute asynchronous tasks,
    so it can run faster when there are a lot of data to compare with
     */

    for {
//      ceo <- CEODao.byId(ceo_id.get)
//      enterprise <- EnterpriseDao.byCEOId(ceo_id.get)
      ceo <- ceo_promise.completeWith(CEODao.byId(ceo_id.get)).future
      enterprise <- enterprise_promise.completeWith(EnterpriseDao.byCEOId(ceo_id.get)).future
    } yield {
      (ceo, enterprise)
    }
  }
